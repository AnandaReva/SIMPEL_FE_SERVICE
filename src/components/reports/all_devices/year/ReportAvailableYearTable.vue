<template>
    <v-container fluid class="px-1 py-4 fill-height" :class="{ 'disable-interactions': isLoading }">
        ...
        <v-row no-gutters class="w-100 align-center mb-4">
            <v-col cols="9">
                <v-select 
                    v-model="selectedOrderByYearsAvailable" 
                    :items="[
...
watch(selectedOrderByYearsAvailable, async (newVal) => {
    year_orderBy.value = newVal === 'tahun' ? 'year' : 'energy';
    resetPagination();
...
const isDisableNextButton = () => {
    ...
    return year_sortType.value === 'desc' 
        ? year_list.value[year_list.value.length - 1].year <= min_year.value
        : year_list.value[year_list.value.length - 1].year >= max_year.value;
};

const isDisablePrevButton = () => {
    if (!year_list.value.length) return true;
    return year_sortType.value === 'desc' 
        ? year_list.value[0].year >= max_year.value
        : year_list.value[0].year <= min_year.value;
};
...
const getReportYearList = async () => {
    ...
    try {
        const operation = "get_report_all_devices_available_years";  // Changed API endpoint
