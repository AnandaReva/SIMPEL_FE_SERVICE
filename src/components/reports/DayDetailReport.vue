<template>
  <v-container fluid class="px-1 py-4 fill-height" :class="{ 'disable-interactions': isLoading }">
    ...
    <v-card color="base" elevation="2" rounded="xl" class="pa-4" style="width: 100%; height: 100%;">
      ...
      <v-container v-if="curr_day_detail">
        ...
        <v-row align="center" justify="space-between" class="mb-4 mx-0">
          ...
          <v-btn @click="backToDailyReport" color="primary" class="d-flex align-center" style="max-height: 50px">
            <v-icon left>mdi-arrow-left-bold</v-icon>
            Kembali ke Laporan Bulan {{ GetMonthNameLocal(curr_month) }}
          ...
          <v-card-title class="text-h5 font-weight-bold text-primary mb-0 px-0">📊 LAPORAN {{
            GetDayNameLocal(curr_date_day).toUpperCase()
          }}, {{ curr_date_day }} {{
              ...
              <v-col cols="12" md="6">
                <v-list density="comfortable">
                  ...
                  <v-list-item>
                    ...
                    <v-list-item-subtitle>{{ curr_day_detail.total_data || '-' }}
                  ...
                  <v-list-item>
                    ...
                    <v-list-item-subtitle>
                      {{
                        FormatCustomDateTime(curr_day_detail.first_record_timestamp, {
                          weekday: true,
                          day: true,
                          month: false,
                          hour: hourOpt = false,
                      ...
                      -
                      {{
                        FormatCustomDateTime(curr_day_detail.last_record_timestamp, {
                          weekday: true,
                          day: true,
                          month: false,
                          hour: hourOpt = false,
                  ...
                  <v-list-item>
                    ...
                    <v-list-item-subtitle>{{ curr_day_detail.data_interval || '-' }}
                  ...
                  <v-list-item>
                    ...
                    <v-list-item-subtitle>{{ FormatSize(curr_day_detail.total_size_bytes) || '-'
                    }}</v-list-item-subtitle>
                  </v-list-item>
                </v-list>
              </v-col>
              ...
              <v-col cols="12" md="6">
                <v-list density="comfortable">
                  ...
                  <v-list-item>
                    ...
                    <v-list-item-subtitle>
                      <v-row>
                        <v-col cols="4">Maks: {{ curr_day_detail.power?.max?.toFixed(2) || '-'
                        }}</v-col>
                        <v-col cols="4">Min: {{ curr_day_detail.power?.min?.toFixed(2) || '-'
                        }}</v-col>
                        <v-col cols="4">Rata-rata: {{ curr_day_detail.power?.avg?.toFixed(2) ||
                          '-' }}</v-col>
                      </v-row>
                    </v-list-item-subtitle>
                  </v-list-item>
                  ...
                  <v-list-item>
                    ...
                    <v-list-item-subtitle>
                      <v-row>
                        <v-col cols="4">Maks: {{ curr_day_detail.voltage?.max?.toFixed(2) ||
                          '-'
                        }}</v-col>
                        <v-col cols="4">Min: {{ curr_day_detail.voltage?.min?.toFixed(2) || '-'
                        }}</v-col>
                        <v-col cols="4">Rata-rata: {{ curr_day_detail.voltage?.avg?.toFixed(2)
                          || '-' }}</v-col>
                      </v-row>
                    </v-list-item-subtitle>
                  </v-list-item>
                  ...
                  <v-list-item>
                    ...
                    <v-list-item-subtitle>
                      <v-row>
                        <v-col cols="4">Maks: {{ curr_day_detail.current?.max?.toFixed(2) ||
                          '-'
                        }}</v-col>
                        <v-col cols="4">Min: {{ curr_day_detail.current?.min?.toFixed(2) || '-'
                        }}</v-col>
                        <v-col cols="4">Rata-rata: {{ curr_day_detail.current?.avg?.toFixed(2)
                          || '-' }}</v-col>
                      </v-row>
                    </v-list-item-subtitle>
                  </v-list-item>
                  ...
                  <v-list-item>
                    ...
                    <v-list-item-subtitle>
                      <v-row>
                        <v-col cols="4">Maks: {{ curr_day_detail.frequency?.max?.toFixed(2) ||
                          '-' }}</v-col>
                        <v-col cols="4">Min: {{ curr_day_detail.frequency?.min?.toFixed(2) ||
                          '-' }}</v-col>
                        <v-col cols="4">Rata-rata: {{
                          curr_day_detail.frequency?.avg?.toFixed(2)
                          || '-' }}</v-col>
                      </v-row>
                    </v-list-item-subtitle>
                  </v-list-item>
                  ...
                  <v-list-item>
                    ...
                    <v-list-item-subtitle>
                      <v-row>
                        <v-col cols="4">Maks: {{ curr_day_detail.power_factor?.max?.toFixed(2)
                          || '-' }}</v-col>
                        <v-col cols="4">Min: {{ curr_day_detail.power_factor?.min?.toFixed(2)
                          ||
                          '-' }}</v-col>
                        <v-col cols="4">Rata-rata: {{
                          curr_day_detail.power_factor?.avg?.toFixed(2) || '-' }}</v-col>
                      </v-row>
                    </v-list-item-subtitle>
                  </v-list-item>
        ...
        <v-card class="mb-6" elevation="2">
          ...
          <div class="table-container">
            ...
            <v-data-table-server :items="hour_list" :items-length="hour_list.length" :items-per-page="-1"
...
const getReportDayDetail = async () => {
  ...
  try {
    const params = {
      ...
      date_day: curr_date_day.value,
      device_id: curr_device.value.id,
    }
    const response = await Process(BASE_API_URL, 'get_report_day_detail', params)
    console.log('getReportDayDetail - params: ', params)
    console.log('getReportDayDetail - response: ', response)
    ...
    hour_list.value = (response.payload.hour_list || [])
      .map(hour => ({
        ...
        data_interval: hour.data_interval,
        energy_consumed_count: hour.energy_consumed_count,
        total_size_bytes: hour.total_size_bytes,
        total_data: hour.total_data,
        power: hour.power,
        voltage: hour.voltage,
        current: hour.current,
        frequency: hour.frequency,
        power_factor: hour.power_factor,
      }))
...
function getYRange(hourList, key) {
  let min = Infinity;
  let max = -Infinity;

  hourList.forEach(hour => {
    if (hour[key]) {
      if (typeof hour[key].min === 'number') {
        min = Math.min(min, hour[key].min);
      }
      if (typeof hour[key].max === 'number') {
        max = Math.max(max, hour[key].max);
      }
    }
  });
...
function renderEnergyChart() {
  ...
  const barChart = new CanvasJS.Chart("energyChart", {
    ...
    theme: "light2",
    title: { text: `Konsumsi Energi ${GetDayNameLocal(curr_date_day.value)}, ${curr_day_detail.value.day_num} ${GetMonthNameLocal(curr_month.value)} ${curr_year.value}` },
    axisY: {
      title: "Energi (kWh)",
      includeZero: true
    },
  ...
  const donutChart = new CanvasJS.Chart("energyDonutChart", {
    animationEnabled: true,
    theme: "light2",
    title: { text: `Proporsi Konsumsi Energi ${GetDayNameLocal(curr_date_day.value)}, ${curr_day_detail.value.day_num} ${GetMonthNameLocal(curr_month.value)} ${curr_year.value}` },
    data: [{
      type: "doughnut",
...
function renderGeneralChart(containerId, titleText, key, label, type = "line", options = {}) {
  ...
  const labels = hour_list.value.map(hour => hour.month_name)
  ...
  const datasets = [
    {
      ...
      dataPoints: labels.map((label, i) => ({
        y: hour_list.value[i][key]?.max ?? null,
        label
      })),
    ...
    },
    {
      ...
      showInLegend: true,
      dataPoints: labels.map((label, i) => ({
        y: hour_list.value[i][key]?.min ?? null,
        label
      })),
    ...
    },
    {
      ...
      showInLegend: true,
      dataPoints: labels.map((label, i) => ({
        y: hour_list.value[i][key]?.avg ?? null,
        label
      })),
  ...
  const chart = new CanvasJS.Chart(containerId, {
    animationEnabled: true,
    theme: "light2",
    title: { text: `${titleText} ${GetDayNameLocal(curr_date_day.value)}, ${curr_day_detail.value.day_num} ${GetMonthNameLocal(curr_month.value)} ${curr_year.value}` },
    axisY: {
      title: label,
...
onMounted(() => {
  ...
  console.log("onMounted - savedDevice: ", savedDevice);

  const yearFromUrl = route.params.year;
  const monthFromUrl = route.params.month;
  const dateDayFromUrl = route.params.date_day;
  if (savedDevice && yearFromUrl && monthFromUrl) {
    ...
    if (parsedDevice && parsedDevice.device && yearFromUrl) {
      ...
      curr_date_day.value = parseInt(dateDayFromUrl);
  ...
  console.log('Date day fromURL:', dateDayFromUrl);

  console.groupEnd();
});

function backToDailyReport() {
