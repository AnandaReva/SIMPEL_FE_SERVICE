<template>
    <v-container fluid class="px-1 py-4 fill-height" :class="{ 'disable-interactions': isLoading }">
        ...
        <v-card color="base" elevation="2" rounded="xl" class="pa-4" style="width: 100%; height: 100%;">
            ...
            <v-container v-if="curr_year_detail">
                ...
                <v-row align="center" justify="space-between" class="mb-4 mx-0">
                    ...
                    <v-btn @click="backToYearlyReportPage" color="primary" class="d-flex align-center"
                            ...
                            <v-col cols="12" md="6">
                                <v-list density="comfortable">
                                    <v-list-item>
                                        ...
                                        <v-list-item-subtitle>{{ curr_year_detail.energy_consumed_count?.toFixed(2) ||
                                            '-' }}
                                    ...
                                    <v-list-item>
                                        ...
                                        <v-list-item-subtitle>{{ curr_year_detail.total_data || '-' }}
                                    ...
                                    <v-list-item>
                                        ...
                                        <v-list-item-subtitle>
                                            {{ curr_year_detail.first_record_timestamp || '-' }} - {{
                                                curr_year_detail.last_record_timestamp ||
                                                '-' }}
                                        </v-list-item-subtitle>
                                    </v-list-item>
                                    ...
                                    <v-list-item>
                                        ...
                                        <v-list-item-subtitle>{{ curr_year_detail.data_interval || '-' }}
                                    ...
                                    <v-list-item>
                                        ...
                                        <v-list-item-subtitle>{{ FormatSize(curr_year_detail.total_size_bytes) || '-'
                                        }}</v-list-item-subtitle>
                                    </v-list-item>
                                </v-list>
                            </v-col>
                            ...
                            <v-col cols="12" md="6">
                                <v-list density="comfortable">
                                    ...
                                    <v-list-item>
                                        ...
                                        <v-list-item-subtitle>
                                            <v-row>
                                                <v-col cols="4">Maks: {{ curr_year_detail.power?.max?.toFixed(2) || '-'
                                                }}</v-col>
                                                <v-col cols="4">Min: {{ curr_year_detail.power?.min?.toFixed(2) || '-'
                                                }}</v-col>
                                                <v-col cols="4">Rata-rata: {{ curr_year_detail.power?.avg?.toFixed(2) ||
                                                    '-' }}</v-col>
                                            </v-row>
                                        </v-list-item-subtitle>
                                    </v-list-item>
                                    ...
                                    <v-list-item>
                                        ...
                                        <v-list-item-subtitle>
                                            <v-row>
                                                <v-col cols="4">Maks: {{ curr_year_detail.voltage?.max?.toFixed(2) ||
                                                    '-'
                                                    }}</v-col>
                                                <v-col cols="4">Min: {{ curr_year_detail.voltage?.min?.toFixed(2) || '-'
                                                }}</v-col>
                                                <v-col cols="4">Rata-rata: {{ curr_year_detail.voltage?.avg?.toFixed(2)
                                                    || '-' }}</v-col>
                                            </v-row>
                                        </v-list-item-subtitle>
                                    </v-list-item>
                                    ...
                                    <v-list-item>
                                        ...
                                        <v-list-item-subtitle>
                                            <v-row>
                                                <v-col cols="4">Maks: {{ curr_year_detail.current?.max?.toFixed(2) ||
                                                    '-'
                                                    }}</v-col>
                                                <v-col cols="4">Min: {{ curr_year_detail.current?.min?.toFixed(2) || '-'
                                                }}</v-col>
                                                <v-col cols="4">Rata-rata: {{ curr_year_detail.current?.avg?.toFixed(2)
                                                    || '-' }}</v-col>
                                            </v-row>
                                        </v-list-item-subtitle>
                                    </v-list-item>
                                    ...
                                    <v-list-item>
                                        ...
                                        <v-list-item-subtitle>
                                            <v-row>
                                                <v-col cols="4">Maks: {{ curr_year_detail.frequency?.max?.toFixed(2) ||
                                                    '-' }}</v-col>
                                                <v-col cols="4">Min: {{ curr_year_detail.frequency?.min?.toFixed(2) ||
                                                    '-' }}</v-col>
                                                <v-col cols="4">Rata-rata: {{
                                                    curr_year_detail.frequency?.avg?.toFixed(2)
                                                    || '-' }}</v-col>
                                            </v-row>
                                        </v-list-item-subtitle>
                                    </v-list-item>
                                    ...
                                    <v-list-item>
                                        ...
                                        <v-list-item-subtitle>
                                            <v-row>
                                                <v-col cols="4">Maks: {{ curr_year_detail.power_factor?.max?.toFixed(2)
                                                    || '-' }}</v-col>
                                                <v-col cols="4">Min: {{ curr_year_detail.power_factor?.min?.toFixed(2)
                                                    ||
                                                    '-' }}</v-col>
                                                <v-col cols="4">Rata-rata: {{
                                                    curr_year_detail.power_factor?.avg?.toFixed(2) || '-' }}</v-col>
                                            </v-row>
                                        </v-list-item-subtitle>
                                    </v-list-item>
                ...
                <v-card class="mb-6" elevation="2">
                    ...
                    <div class="table-container">
                        ...
                        <v-data-table-server :items="month_list" :items-length="month_list.length" :items-per-page="-1"
...
import { FormatSize, FormatToLocal, GetMonthNameLocal } from '@/utils/utils';
...
const getReportMonthList = async () => {
    ...
    try {
        ...
        popUpProps.value = {
            status: 'error',
            errorMessage: 'Gagal mendapatkan data tahun',
...
function getYRange(monthList, key) {
    let min = Infinity;
    let max = -Infinity;

    monthList.forEach(month => {
        if (month[key]) {
            if (typeof month[key].min === 'number') {
                min = Math.min(min, month[key].min);
            }
            if (typeof month[key].max === 'number') {
                max = Math.max(max, month[key].max);
            }
        }
    });
...
function renderEnergyChart() {
    ...
    const barChart = new CanvasJS.Chart("energyChart", {
        ...
        theme: "light2",
        title: { text: `Konsumsi Energi Bulanan Tahun ${curr_year.value}` },
        axisY: {
            title: "Energi (kWh)",
            includeZero: true
        },
...
function renderGeneralChart(containerId, titleText, key, label, type = "line", options = {}) {
    ...
    const labels = month_list.value.map(month => month.month_name)
    ...
    const datasets = [
        {
            ...
            dataPoints: labels.map((label, i) => ({
                y: month_list.value[i][key]?.max ?? null,
                label
            })),
        ...
        },
        {
            ...
            showInLegend: true,
            dataPoints: labels.map((label, i) => ({
                y: month_list.value[i][key]?.min ?? null,
                label
            })),
        ...
        },
        {
            ...
            showInLegend: true,
            dataPoints: labels.map((label, i) => ({
                y: month_list.value[i][key]?.avg ?? null,
                label
            })),
    ...
    const chart = new CanvasJS.Chart(containerId, {
        animationEnabled: true,
        theme: "light2",
        title: { text: `${titleText} Tahunan ${curr_year.value}` },
        axisY: {
            title: label,
...
function backToYearlyReportPage() {
