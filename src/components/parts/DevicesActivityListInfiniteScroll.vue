<template>
    <v-container class="activity-list px-0 ma-0">
        ...
        <v-list bg-color="white" dense class="rounded-lg elevation-2">
            ...
            <!-- Aktivitas -->
            <template v-if="device_activities && device_activities.length > 0">
                <template v-for="(activity, index) in device_activities" :key="activity.activity_id">
                    ...
                    <v-divider v-if="index < device_activities.length - 1" class="mx-4" />
                </template>
            </template>
...
*/
const props = defineProps(["device_activities"]);
